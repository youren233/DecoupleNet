...
class JointsLambdaDatasetSKT(Dataset):
    ...
    def getitem_helper(self, db_rec, another_db_rec=None):
        ...
        # two skt info
        skt_file = os.path.join(os.path.dirname(image_file) + "_anns_skt",
                                  filename[:-4] + '_' + str(db_rec['annotation_id']) + '_skt.jpg')
        another_skt_file = os.path.join(os.path.dirname(image_file) + "_anns_skt",
                                  filename[:-4] + '_' + str(another_db_rec['annotation_id']) + '_skt.jpg')
        ...
        skt_numpy = cv2.imread(skt_file, cv2.IMREAD_GRAYSCALE)
        skt_numpy = np.expand_dims(skt_numpy, axis=2)

        another_skt_numpy = cv2.imread(another_skt_file, cv2.IMREAD_GRAYSCALE)
        another_skt_numpy = np.expand_dims(another_skt_numpy, axis=2)
        ...
        if another_joints_vis[:, 0].sum() <= 3:
            ...
            another_skt_numpy = skt_numpy
        # -------------------------------

        # cat depth
        data_numpy = np.concatenate((data_numpy, skt_numpy, another_skt_numpy), 2)
