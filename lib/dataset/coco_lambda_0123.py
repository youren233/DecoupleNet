...
from lib.dataset.JointsLambdaGeneralDataset import JointsLambdaGeneralDataset
from lib.nms.nms import oks_nms
from lib.nms.nms import soft_oks_nms
from lib.utils.utils import compute_ious

logger = logging.getLogger(__name__)


class COCODataset(JointsLambdaGeneralDataset):
    '''
    "keypoints": {
        0: "nose",
        1: "left_eye",
    ...
    ## target_mode:
    ## background: loss on background
    ## foreground: loss on foreground
    ## random: randomly choose between foreground and background
    ...
    def _load_coco_keypoint_annotation_kernal(self, index):
        ...
        objs = valid_objs ## valid objects for the image at index

        ## ---- now compute nn to each bounding box with respect to center------        

        for idx, obj in enumerate(objs):

            dist_to_other_obj = {} ## annotation idx: dist
            ...
            for other_idx, other_obj in enumerate(objs):
                ...
                dist_to_other_obj[other_obj['id']] = dist

            ##-------sort by ascending distance----------
            new_dist_to_other_obj = {k: v for k, v in sorted(dist_to_other_obj.items(), key=lambda item: item[1])}

            obj['distance_sorted_annotation_idxs'] = list(new_dist_to_other_obj.keys())
        ...
        for idx, obj in enumerate(objs):
            ...
            # ---------------------------------------
            # num_candidates = 2 ## for 0, 1, 2
            num_candidates = 3 ## for 0, 1, 2, 3

            candidate_annotation_ids = obj['distance_sorted_annotation_idxs'][:num_candidates] ##
            candidate_db_recs = [None]*num_candidates

            for candidate_num, candidate_annotation_id in enumerate(candidate_annotation_ids):
                candidate_annotation = self.get_obj_by_annotation_id(objs, candidate_annotation_id)
                candidate_db_rec = self._only_load_coco_keypoint_annotation(candidate_annotation, im_ann)[0]
                candidate_db_recs[candidate_num] = candidate_db_rec
            ...
            rec.append({
                ...
                'candidate_db_recs': candidate_db_recs
            })

        return rec

    def get_obj_by_annotation_id(self, objs, annotation_id):
        return_obj = None
