    ...
    def __init__(self, cfg, **kwargs):
        ...
        if self.se_config[0]:
            self.stage1_se = SELambdaModule(channel_list=self.stage2_cfg['NUM_CHANNELS'],
                                            feature_size_list=self.stage2_cfg['FEATURE_SIZES'])
        if self.se_config[1]:
            self.stage2_se = SELambdaModule(channel_list=self.stage3_cfg['NUM_CHANNELS'],
                                            feature_size_list=self.stage3_cfg['FEATURE_SIZES'])
        if self.se_config[2]:
            self.stage3_se = SELambdaModule(channel_list=self.stage4_cfg['NUM_CHANNELS'],
                                            feature_size_list=self.stage4_cfg['FEATURE_SIZES'])
        if self.se_config[3]:
            self.stage4_se = SELambdaModule(channel_list=[self.stage4_cfg['NUM_CHANNELS'][0]],
                                            feature_size_list=[self.stage4_cfg['FEATURE_SIZES'][0]]) ## last stage
...
class SELambdaLayer(nn.Module):
    def __init__(self, channel, feature_size, reduction=4):
        ...
        self.h_lambda = nn.Sequential(
            nn.Linear(2, channel // reduction),
            nn.BatchNorm1d(channel // reduction),
            nn.ReLU(inplace=True),
            nn.Linear(channel // reduction, feature_size[0]),
        )
        self.w_lambda = nn.Sequential(
            nn.Linear(2, channel // reduction),
            nn.BatchNorm1d(channel // reduction),
            nn.ReLU(inplace=True),
            nn.Linear(channel // reduction, feature_size[1]),
        )
        self.sig = nn.Sigmoid()
    ...
    def forward(self, x, lambda_vec):
        ...
        z = self.cs_lambda(lambda_vec).view(b, c, 1, 1)
        h = self.h_lambda(lambda_vec).view(b, 1, -1, 1)
        w = self.w_lambda(lambda_vec).view(b, 1, 1, -1)
        # spatial selector
        s = self.sig(h * w)

        out = x * y.expand_as(x) * z.expand_as(x) * s
        return out

# ---------------------------------------------------
class SELambdaModule(nn.Module):
    def __init__(self, channel_list, feature_size_list):
        super(SELambdaModule, self).__init__()
        self.se_layers = []

        for i in range(len(channel_list)):
            feature_size = feature_size_list[i].split('x')
            se_layer = SELambdaLayer(channel=channel_list[i], feature_size=[int(s) for s in feature_size], reduction=4)
