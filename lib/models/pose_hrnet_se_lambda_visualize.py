...
class PoseHighResolutionNet(nn.Module):
    ...
    def forward(self, x, lambda_vec):
        ...
        all_se_list = []
        all_embedding_list = []
        all_before_x_list = [] ## stores tensors coming out
        all_after_x_list = [] ## stores tensors coming out
        ...
        # -------------------
        if self.se_config[0]:
            all_before_x_list.extend(x_list)
            x_list, se_list, embedding_list = self.stage1_se(x_list, lambda_vec) ## 2, (32, 64), space_res = 64 x 48
            all_se_list.extend(se_list)
            all_embedding_list.extend(embedding_list)
            all_after_x_list.extend(x_list)
        ...
        if self.se_config[1]:
            all_before_x_list.extend(x_list)
            x_list, se_list, embedding_list = self.stage2_se(x_list, lambda_vec) ## 3, (32, 64, 128) , space_res = 64 x 48
            all_se_list.extend(se_list)
            all_embedding_list.extend(embedding_list)
            all_after_x_list.extend(x_list)
        # -------------------
        ...
        if self.se_config[2]:
            all_before_x_list.extend(x_list)
            x_list, se_list, embedding_list = self.stage3_se(x_list, lambda_vec)  ## 4, (32, 64, 128, 256) , space_res = 64 x 48
            all_se_list.extend(se_list)
            all_embedding_list.extend(embedding_list)
            all_after_x_list.extend(x_list)
        # -------------------
        ...
        # -------------------
        if self.se_config[3]:
            all_before_x_list.extend(y_list)
            y_list, se_list, embedding_list = self.stage4_se(y_list, lambda_vec)  ## 1, (32) , space_res = 64 x 48
            all_se_list.extend(se_list)
            all_embedding_list.extend(embedding_list)
            all_after_x_list.extend(y_list)
        # -------------------

        x = self.final_layer(y_list[0])

        return x, all_se_list, all_embedding_list, all_before_x_list, all_after_x_list
...
class SELambdaLayer(nn.Module):
    ...
    def forward(self, x, lambda_vec):
        ...
        return out, y.view(b, c), z.view(b, c)
...
class SELambdaModule(nn.Module):
    ...
    def forward(self, x_list, lambda_vec):
        out_list_se = []
        y_list_se = []
        z_list_se = []

        for i in range(len(x_list)):
            out_se, y_se, z_se = self.se_layers[i](x_list[i], lambda_vec)
            out_list_se.append(out_se)
            y_list_se.append(y_se)
            z_list_se.append(z_se)

        return out_list_se, y_list_se, z_list_se
