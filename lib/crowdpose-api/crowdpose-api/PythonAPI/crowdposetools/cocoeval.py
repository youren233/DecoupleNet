...
class Params:
    ...
    def setDetParams(self):
        ...
        n = np.round((0.95 - .5) / .05) + 1
        self.iouThrs = np.linspace(.5, 0.95, n.astype(int), endpoint=True)
        m = np.round((1.00 - .0) / .01) + 1
        self.recThrs = np.linspace(.0, 1.00, m.astype(int), endpoint=True)
